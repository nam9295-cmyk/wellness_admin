      ...
      <PageSection title="운영 설정" description="역할과 organization 스코프를 바꾸며 관리자 흐름을 바로 테스트할 수 있습니다">
        <div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-2xl border border-slate-200 p-5">
            <h4 className="text-base font-semibold text-slate-900">현재 접근 스코프</h4>
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-700" htmlFor="role-select">
                  현재 역할
                </label>
                <select
                  id="role-select"
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-700" htmlFor="organization-select">
                  organizationId
                </label>
                <select
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-900 outline-none transition disabled:cursor-not-allowed disabled:opacity-60 focus:border-teal-500"
                >
            ...
            <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">
              <p>현재 역할: <span className="font-semibold text-slate-900">{role}</span></p>
              <p className="mt-1">현재 organization: <span className="font-semibold text-slate-900">{organizationId ?? '없음'}</span></p>
            </div>
          </div>

          <div className="rounded-2xl border border-slate-200 p-5">
            <h4 className="text-base font-semibold text-slate-900">테스트 데이터 생성</h4>
            ...
            <button
              ...
              disabled={isSeeding}
              className="mt-4 rounded-xl bg-teal-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-teal-700 disabled:cursor-not-allowed disabled:opacity-60"
            >
